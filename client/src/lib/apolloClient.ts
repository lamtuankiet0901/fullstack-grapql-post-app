...
function createApolloClient(headers: IncomingHttpHeaders | null = null) {
  ...
  const httpLink = new HttpLink({
    uri: process.env.NODE_ENV === 'production' ? 'https://vast-mesa-09579.herokuapp.com/graphql' : 'http://localhost:4000/graphql', // Server URL (must be absolute)
...
export function initializeApollo({headers, initialState}: {
  ...
  initialState?: NormalizedCacheObject | null} = {headers: null, initialState: null}) {
